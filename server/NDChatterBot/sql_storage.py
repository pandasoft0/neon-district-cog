...
class SQLStorageAdapter(StorageAdapter):
    ...
    def filter(self, **kwargs):
        ...
        if len(filter_parameters) == 0:
            ...
            for i, fp in enumerate(filter_parameters):
                ...
                elif fp in ['continue_response_to']:
                    Response_alias = aliased(Response)
                    _response_query = session.query(Statement)
                    _query = _response_query.join(Response).filter(Response.text == _filter[0])
                    #_query = _query.join(Response_alias, Response.text == Response_alias.statement_text).filter(Response_alias.text == _filter[1]).order_by(func.abs(Response.id - Response_alias.id))
                    _query = _query.join(Response_alias, Response.text == Response_alias.statement_text).filter(Response_alias.text == _filter[1]).filter(Response.id - 1 == Response_alias.id)

                    sq_res = _query.subquery()
                    _query = session.query(Statement).filter(sq_res.c.id == Statement.id)
                else:
                    if _query:
    ...
    def get_prior_responses(self, conversation_id):
        """
        Returns the latest response in a conversation if it exists.
        Returns None if a matching conversation cannot be found.
        """
        ...
        if statement_query.count() <= 5:
            for i in range(statement_query.count()):
                statements.append(statement_query[i].get_statement())
        else:
            statement_query[-5:]
            for st in statement_query:
                statements.append(st.get_statement())
